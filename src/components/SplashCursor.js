...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvasEl) {
      ...
      const halfFloatTexType = isWebGL2 ? WebGL2RenderingContext.HALF_FLOAT : halfFloat && halfFloat.HALF_FLOAT_OES;
      if (!halfFloatTexType) throw new Error("Half float texture type not supported");
    ...
    function initFramebuffers() {
      ...
      const r = ext.formatR;
      if (!rgba || !rg || !r) throw new Error("Required texture formats not supported");
    ...
    function generateColor() {
      ...
      const value = 0.1 + Math.random() * 0.2;
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        if (target && !target.closest("a, button, input, textarea, select")) {
          setTimeout(() => {
            if (!pointers[0].down) clearCanvas();
          }, 100);
        }
      },
      { passive: true }
  ...
  return (
    ...
    >
      <canvas ref={canvasRef} id="fluid" style={{ width: "100%", height: "100%" }} />
    </div>
  );
}
